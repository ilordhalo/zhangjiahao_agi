...
Report links always use `public_base_url`, even when a temporary port differs; startup warns about that mismatch. Configs matching the generated pre-0.4 layout exactly retain the existing explicit `symphonz run --port 4000` flow through a temporary migration bridge. That bridge is unauthenticated, requires an explicit port, and is restricted to a loopback bind; wildcard and LAN hosts are rejected. Configured, modified, or ambiguous files never use the bridge: dashboard configurations require `auth.toml`, while other noncanonical files fail closed with instructions to run `symphonz configure-dashboard`. Run that command promptly to remove the legacy exception and enable authentication.
