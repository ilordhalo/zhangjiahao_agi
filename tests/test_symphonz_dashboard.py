...
class DashboardTests(unittest.TestCase):
    ...
    def test_authenticated_pages_render_chinese_first_content_escaped_and_responsive(self):
        ...
        self.assertIn(".grid-two > *, .section { min-width: 0; }", overview.text)
        self.assertIn(".grid-two { grid-template-columns: minmax(0, 1fr); }", overview.text)
        self.assertIn(".table-wrap { max-width: 100%; overflow-x: auto; }", overview.text)
