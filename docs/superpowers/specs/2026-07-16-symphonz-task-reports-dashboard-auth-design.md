...
Publishing is idempotent for an issue. The publisher keeps its non-symlink artifact-root descriptor pinned for its full lifetime, persists only relative generation names in RuntimeStore, and exposes safe current-JSON/current-HTML reads through that pinned root. It writes a new `report-<generation>.json` and `report-<generation>.html` bundle through relative `dir_fd` operations, and fsyncs both files and the directory. Only after both files exist does RuntimeStore atomically switch its authoritative JSON/HTML names. A failed second write leaves the previous names unchanged; a successful commit removes every unreferenced generation, including interrupted or concurrent leftovers. Task 4, not Task 3, implements the authenticated stable route by resolving the RuntimeStore-authoritative HTML generation through this safe read interface:
